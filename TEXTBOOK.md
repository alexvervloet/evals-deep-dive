...
An interval that excludes zero answers only whether the observed lift is distinguishable from sampling variation under the design. It does not answer whether the lift matters. The product team must declare a **minimum practical effect** before seeing outcomes: perhaps three percentage points of task completion are needed to pay for higher latency and migration risk. Example 14 deliberately finds a precisely estimated two-point lift. Its interval clears zero, so the evidence supports a statistical improvement, but the whole interval remains below the three-point boundary, so the decision is hold. This is not semantics; the [American Statistical Association's statement](https://www.amstat.org/asa/files/pdfs/p-valuestatement.pdf) explicitly warns that statistical significance does not measure effect size or importance.
...
The tool runs a suite, saves a baseline, diffs new runs against it (using `compare()` as a fixed-horizon screen so it does not cry regression over every numeric change), and, decisively, exits nonzero when quality drops below a threshold. That exit code is the whole trick. It means a CI system can block a merge on it, which converts evaluation from a virtue people are encouraged to practice into a fact about what can reach production. Nobody remembers to run quality checks under deadline; gates do not need to remember. The capstone's aggregate diff is still a teaching shortcut; a release gate comparing the same cases should retain per-case scores and apply the predeclared policy from Section 5.9.
